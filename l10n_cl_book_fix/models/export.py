...
class BookXlsx(models.AbstractModel):
    _name = 'report.l10n_cl_book_fix.book_xlsx'
    _inherit = 'report.report_xlsx.abstract'

    def generate_xlsx_report(self, workbook, data, book):
        for obj in book:
            ...
            for mov in obj.move_ids:
                ...
                total_afecto += totales['neto']
                total_exento += totales['exento']
                total_iva += totales['iva']
                line += 1

            sheet.write(line, 0, "Total General", bold)
            sheet.write(line, 5, total_afecto, bold)
            sheet.write(line, 6, total_exento, bold)
            sheet.write(line, 7, total_iva, bold)
            c = 8
