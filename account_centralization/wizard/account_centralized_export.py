...
class account_move_excel_wizard_form(models.TransientModel):
    ...
    date_to = fields.Date('Period', required=True)
    libro_id = fields.Many2one('account.journal', 'Account Journal')
    ...
    def account_centralized_history_excel(self):
        ...
        worksheet.write_merge(0, 0, 2, 5, "ACCOUNT CENTRALIZATION", style)
        worksheet.write(1, 2, 'Month', style)
        worksheet.write(1, 3, mes, style)
        worksheet.write(1, 4, 'Journal', style)
        worksheet.write(1, 5, self.libro_id.name, style)
        worksheet.write(2, 2, 'Year', style)
        ...
        self.env.cr.execute("""SELECT sum(balance) as suma,  code,  account_account.name as nombre, account_move_line.analytic_account_id as aa FROM account_move_line left join account_account
on  account_account.id = account_move_line.account_id  where (to_char(date,'mm')= %s) and (to_char(date,'yyyy')= '%s') and journal_id='%s' GROUP BY code,  account_account.name, account_move_line.analytic_account_id """,(mes,ano,libro))
        ...
        worksheet.write_merge(n-1, n-1, 2, 3, 'Account', style1)
        worksheet.write(n-1, 4, 'Analytic Account', style1)
        worksheet.write(n-1, 5, 'Debit', style1)
        worksheet.write(n-1, 6, 'Credit', style1)
        ...
        for rec in line_list:
            ...
            cc = self.env['account.analytic.account'].search([('id', '=', rec['aa'])])
            if cc:
                analytic_account= cc.name
            else:
                analytic_account= ''
            
            worksheet.write(n, c+1, rec['code'], style)
            worksheet.write(n, c+2, rec['nombre'], style)
            worksheet.write(n, c+3, analytic_account, style)

            

            if rec['suma'] > 0:
                worksheet.write(n, c+4, rec['suma'], currency)            
                debe = debe + float(rec['suma'])
            else:
                worksheet.write(n, c+5, float(rec['suma']) * -1, currency)       
                haber = haber + (float(rec['suma']) * -1)
            n = n+1


        worksheet.write(n,5, debe, currency)
        worksheet.write(n,6, haber, currency)

        worksheet.write(n+3,2, "Vouchers:", style)
